...
func (app *App) initDB() {
	...
	app.db.Exec(`CREATE TABLE IF NOT EXISTS categories (
		...
		UNIQUE(name, parent_id, username)
	)`)
	// 迁移：旧表可能没有 parent_id
	app.db.Exec(`ALTER TABLE categories ADD COLUMN parent_id INTEGER DEFAULT 0`)
...
func (app *App) getSession(r *http.Request) *Session {
	...
	if cookie, err := r.Cookie("session"); err == nil {
		token = cookie.Value
	} else if h := r.Header.Get("X-Session-Token"); h != "" {
		token = h
	}
...
func (app *App) handleHardDeletePost(w http.ResponseWriter, r *http.Request) {
	username, _ := app.requireAuth(w, r)
	if username == "" {
		return
	}

	idStr := strings.TrimPrefix(strings.TrimSuffix(r.URL.Path, "/hard_delete"), "/api/posts/")
	var postID int64
	fmt.Sscanf(idStr, "%d", &postID)
	...
	// Delete from DB
	app.db.Exec("DELETE FROM attachments WHERE post_id = ?", postID)
	app.db.Exec("DELETE FROM posts WHERE id = ? AND author = ?", idStr, username)

	app.broadcastToUser(username, map[string]interface{}{"type": "post_deleted", "data": map[string]string{"id": idStr}})
	jsonResponse(w, map[string]string{"message": "已彻底删除"})
}
...
func (app *App) handleRestorePost(w http.ResponseWriter, r *http.Request) {
	...
	id := strings.TrimPrefix(strings.TrimSuffix(r.URL.Path, "/restore"), "/api/posts/")
	app.db.Exec("UPDATE posts SET is_deleted = 0, deleted_at = '' WHERE id = ? AND author = ?", id, username)

	// Optional: Broadcast restore
	app.broadcastToUser(username, map[string]interface{}{"type": "post_updated", "data": map[string]string{"id": id}})
...
func (app *App) handleFile(w http.ResponseWriter, r *http.Request) {
	...
	// Hardcode fallbacks to prevent DetectContentType from renaming APKs to ZIPs, or breaking images
	...
	// Inject secure filename attachment header if specific `?download=` attribute triggers
	downloadName := r.URL.Query().Get("download")
	if downloadName != "" {
		// RFC 5987 compliant UTF-8 filename encoding for strict mobile browsers
	...
	// Remove isolated nosniff to allow WebView sniffing if detection fails loosely
...
func (app *App) handleGetCategories(w http.ResponseWriter, r *http.Request) {
	...
	rows, _ := app.db.Query(`SELECT id, name, parent_id, sort_order FROM categories WHERE username = ? ORDER BY sort_order, id`, username)
	defer rows.Close()
	...
	for rows.Next() {
		n := &catNode{Children: []*catNode{}}
		rows.Scan(&n.ID, &n.Name, &n.ParentID, &n.Order)
		all = append(all, n)
		byID[n.ID] = n
	}

	// 统计每个分类的文章数
	countRows, _ := app.db.Query(`SELECT category, COUNT(*) FROM posts WHERE author = ? AND is_deleted = 0 GROUP BY category`, username)
	countMap := map[string]int{}
	if countRows != nil {
		for countRows.Next() {
			var cat string
			var cnt int
			countRows.Scan(&cat, &cnt)
			countMap[cat] = cnt
		}
		countRows.Close()
	}
	for _, n := range all {
		n.Count = countMap[n.Name]
	}
	...
	for _, n := range all {
		if n.ParentID == 0 {
			...
			roots = append(roots, n) // 父节点不存在，当根节点
		}
	}
	if roots == nil {
		roots = []*catNode{}
...
func (app *App) handleCreateCategory(w http.ResponseWriter, r *http.Request) {
	...
	result, err := app.db.Exec(`INSERT INTO categories (name, parent_id, username, sort_order) VALUES (?, ?, ?, ?)`,
		req.Name, req.ParentID, username, req.Order)
	if err != nil {
		jsonError(w, "分类已存在", http.StatusConflict)
		return
	}
	id, _ := result.LastInsertId()
	jsonResponse(w, map[string]interface{}{"id": id, "name": req.Name, "parent_id": req.ParentID})
}
...
func (app *App) handleUpdateCategory(w http.ResponseWriter, r *http.Request) {
	...
	// 如果改名，同步更新文章的分类字段
	var oldName string
	app.db.QueryRow("SELECT name FROM categories WHERE id = ? AND username = ?", id, username).Scan(&oldName)
	if req.Name != "" && req.Name != oldName {
		req.Name = clampString(req.Name, 64)
		app.db.Exec("UPDATE posts SET category = ? WHERE category = ? AND author = ?", req.Name, oldName, username)
	} else if req.Name == "" {
		req.Name = oldName
	}

	if req.ParentID != nil {
		app.db.Exec("UPDATE categories SET name = ?, parent_id = ?, sort_order = ? WHERE id = ? AND username = ?",
			req.Name, *req.ParentID, req.Order, id, username)
	} else {
		app.db.Exec("UPDATE categories SET name = ?, sort_order = ? WHERE id = ? AND username = ?",
			req.Name, req.Order, id, username)
	}
	jsonResponse(w, map[string]string{"message": "已更新"})
}
...
func (app *App) handleDeleteCategory(w http.ResponseWriter, r *http.Request) {
	...
	var name string
	var parentID int64
	app.db.QueryRow("SELECT name, parent_id FROM categories WHERE id = ? AND username = ?", id, username).Scan(&name, &parentID)

	// 子分类提升到父级
	app.db.Exec("UPDATE categories SET parent_id = ? WHERE parent_id = ? AND username = ?", parentID, id, username)
	// 该分类下的文章归为未分类
...
func (app *App) handleGetTags(w http.ResponseWriter, r *http.Request) {
	...
	// 从文章中动态聚合标签及其使用次数
	rows, _ := app.db.Query(`SELECT tags FROM posts WHERE author = ? AND is_deleted = 0 AND tags != ''`, username)
	defer rows.Close()
...
func (app *App) handleCreateTag(w http.ResponseWriter, r *http.Request) {
	...
	result, err := app.db.Exec("INSERT OR IGNORE INTO tags (name, username) VALUES (?, ?)", req.Name, username)
	if err != nil {
		jsonError(w, "创建失败", http.StatusInternalServerError)
		return
	}
	id, _ := result.LastInsertId()
...
func (app *App) handleLanIPs(w http.ResponseWriter, r *http.Request) {
	// 不需要登录验证，但返回的信息不敏感（只是IP列表）
	var ips []string
	...
	jsonResponse(w, map[string]interface{}{"ips": ips, "port": port})
}
...
func (app *App) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(path, "/api/") {
		...
		w.Header().Set("Access-Control-Allow-Headers", "Content-Type, X-API-Token, X-Session-Token")
	...
	if strings.HasPrefix(path, "/api/") {
		...
		case strings.HasPrefix(path, "/api/posts/") && strings.HasSuffix(path, "/restore") && r.Method == "POST":
			app.handleRestorePost(w, r)
		case strings.HasPrefix(path, "/api/posts/") && strings.HasSuffix(path, "/hard_delete") && r.Method == "DELETE":
			app.handleHardDeletePost(w, r)
		...
		case path == "/api/lan-ips" && r.Method == "GET":
			app.handleLanIPs(w, r)
