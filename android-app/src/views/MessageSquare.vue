          ...
          <div class="msg-card">
            <div class="msg-head">
              ...
              <div class="head-actions">
                ...
                <button v-if="(m.attachments && m.attachments.length > 0) || m.file_url" class="inline-action-btn" @click.stop="downloadAllMsgFiles(m)" title="下载">
...
async function downloadAllMsgFiles(m) {
  const atts = m.attachments || (m.file_url ? [{url: m.file_url, name: m.file_name}] : []);
  if (!atts.length) return;
  if (atts.length === 1) {
    openBrowser(fileURL('/api/file/' + atts[0].url));
    return;
  }
  
  toast(`开始打包获取 ${atts.length} 个文件...`);
  for (let i = 0; i < atts.length; i++) {
    const url = fileURL('/api/file/' + atts[i].url);
