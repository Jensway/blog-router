...
func (app *App) initDB() {
	...
	// 为已有数据库添加 content_type 字段（如果已存在会报错，这里忽略）
...
func (app *App) setSessionCookie(w http.ResponseWriter, token string, maxAge int) {
	c := &http.Cookie{
		Name:     "session",
		Value:    token,
		Path:     "/",
	...
	if maxAge > 0 && w != nil {
		// 仅在 https 下启用 Secure，避免 http 开发环境登录不了
		// Go 的 Cookie.Secure 由调用方判断 r.TLS 更准确，但此处仅用于输出安全默认值
	}
	http.SetCookie(w, c)
}
...
// sanitizeForRender: 输出侧统一净化（方案A）
// - 普通文本：HTML 转义 + \n 转 <br>
// - 支持 ```lang?\ncode``` 代码块：code 内部 HTML 转义，输出 <pre><code class="language-...">...</code></pre>
// - 限制最大输出长度，避免极端 payload 造成前端卡死
func sanitizeForRender(input string) string {
	if input == "" {
		return ""
	}
	// 防止超大内容导致前端渲染卡死（可按需调大）
...
func (app *App) handleGetPosts(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		// 列表不返回正文（原逻辑），但也不返回 safe_content
		p.Content = ""
		p.SafeContent = ""
...
func (app *App) handleGetPost(w http.ResponseWriter, r *http.Request) {
	...
	// 方案A：返回 safe_content（供前端 innerHTML 安全渲染）
...
func (app *App) handleCreatePost(w http.ResponseWriter, r *http.Request) {
	...
	// 允许前端只传部分字段时不报 unknown，但仍限制 body 大小
...
func (app *App) handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	if postID == 0 {
		...
		result, _ := app.db.Exec(`INSERT INTO posts (title, content, content_type, category, tags, is_draft, source, author, created_at, updated_at) 
						VALUES (?, '', 'text', '', '', 0, ?, ?, ?, ?)`, filename, source, username, now, now)
...
func (app *App) handleUploadMerge(w http.ResponseWriter, r *http.Request) {
	...
	if postID == 0 {
		...
		result, _ := app.db.Exec(`INSERT INTO posts (title, content, content_type, category, tags, is_draft, source, author, created_at, updated_at) 
			VALUES (?, '', 'text', '', '', 0, ?, ?, ?, ?)`, req.Filename, source, username, now, now)
		postID, _ = result.LastInsertId()
	}
...
func (app *App) handleFile(w http.ResponseWriter, r *http.Request) {
	...
	// 附件权限校验：只有作者(或管理员)能下载
	var author string
	var origName string
	err := app.db.QueryRow(`SELECT p.author, a.orig_name
		FROM attachments a JOIN posts p ON a.post_id = p.id
		WHERE a.filename = ?`, filename).Scan(&author, &origName)
	if err != nil {
		http.NotFound(w, r)
		return
	}
	if !sess.IsAdmin && author != sess.Username {
		http.Error(w, "无权限", http.StatusForbidden)
		return
	}

	origName = sanitizeFilenameForDownload(origName)
	...
	// inline 更友好：图片/pdf 可直接预览
	w.Header().Set("Content-Disposition", fmt.Sprintf(`inline; filename="%s"`, strings.ReplaceAll(origName, `"`, "")))

	http.ServeFile(w, r, filepath.Join(app.workDir, "uploads", filename))
}
...
func (app *App) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	// 基础安全头（不强上 CSP，避免你依赖 CDN/Jodit 被误伤；需要的话我再给“可配置 CSP”）
	w.Header().Set("X-Content-Type-Options", "nosniff")
...
func main() {
	...
	if !cmdPortSet && app.settings.Port > 0 {
	...
	for {
		...
		log.Printf("博客系统启动: http://0.0.0.0:%d", port)
		log.Printf("工作目录: %s", workDirFlag)
		log.Printf("默认管理员: admin / admin")
